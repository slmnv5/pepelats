...
class Song:
    ...
    _ff = FileFinder(AppName.save_song, True, ".sng")
    ...
    @staticmethod
    def show_list() -> str:
        return Song._ff.get_str(SCR_ROWS - 5)

    @staticmethod
    def delete_song() -> None:
        Song._ff.delete_item()

    @staticmethod
    def get_prev() -> str:
        return Song._ff.get_prev()

    @staticmethod
    def get_next() -> str:
        return Song._ff.get_next()

    @staticmethod
    def load_song_drum() -> tuple['Song', BaseDrum]:
        song = Song()
        song._name = Song._ff.get_item().split(".")[0]
        fname = Song._ff.get_full_name()
    ...
    def save_song_drum(self, drum: BaseDrum) -> None:
        Song._ff.add_item(self.get_complete_name(drum))
        fname = Song._ff.get_full_name()
        parts_lst = list()
        self.parts.for_each(lambda x: parts_lst.append(None if x.is_empty() else x))
