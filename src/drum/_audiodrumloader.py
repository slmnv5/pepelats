...
from utils import JsonDict, SD_TYPE, ConfigName
from utils import SD_MAX
from utils.log import LOGGER
from utils.utilalsa import make_zero_buffer, record_sound_buff
...
class AudioDrumLoader(FakeDrum, ABC):
    ...
    def __prepare_drum(self, length: int) -> None:
        ...
        LOGGER.info(f"Generated drum patterns {len(self.__snd_l1)}")
    ...
    def __load(self) -> None:
        ...
        LOGGER.info(f"Loading drum {directory}")
        if len(self.__sounds) == 0:
            self.__load_sounds(directory)
            LOGGER.info(f"Loaded drum sounds {len(self.__sounds)}")
        ...
        LOGGER.info(f"Loaded drum patterns {len(self.__ptn_l1)}")

    def __load_sounds(self, directory: str) -> None:
        """Loads WAV sounds"""
        path = os.path.join(directory, "./../drum_sounds.json")
        ...
        for name in loader.dic():
            ...
            (sound, _) = sf.read(file_name, dtype="int16", always_2d=True)
            assert sound.ndim == 2
    ...
    def __prepare_one(self, pattern, length: int) -> np.ndarray:
        accents = pattern["acc"]
        numpy_array = make_zero_buffer(length)
        for sound_name in [x for x in self.__sounds if x in pattern]:
            ...
            for step_number in range(steps):
                if notes[step_number] != '.':
                    ...
                    record_sound_buff(numpy_array, tmp, pos)
